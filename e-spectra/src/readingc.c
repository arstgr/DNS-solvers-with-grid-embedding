...
POINTER readingc(int xdc, int ydc, int zdc, POINTER V, PDATA pd, int ts)
{
	char fn[40];
	int cntr,b;
	MPI_File fh;
	...
	off1 = (MPI_Offset)(zdc);
	off2 = (MPI_Offset)(ydc)*off1;
	offset=((MPI_Offset)b)*((MPI_Offset)(xdc-1))*((MPI_Offset)3)*off2*sizeof(double);
	MPI_File_set_view(fh,offset,MPI_DOUBLE,MPI_DOUBLE,"native",MPI_INFO_NULL);
	MPI_File_read(fh,(V.velc),((xdc-1)*ydc*zdc*3),MPI_DOUBLE,&status);
